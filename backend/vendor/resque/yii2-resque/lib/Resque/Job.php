...
class Resque_Job
{
	...
	public function __construct($queue, $payload)
	{
		...
		if(file_exists(\yii\BaseYii::$app->basePath.'/../app/components')){
			 // yii 2 advanced
                $this->payload['class'] = 'app\\components\\job\\'.$this->payload['class'];
        }
        else{
        	// yii2 basic
        	 $this->payload['class'] = 'app\\components\\job\\'.$this->payload['class'];
...
//				);
                            echo 'Could not find job class ' . $this->payload['class'] . '.';die;
			}
